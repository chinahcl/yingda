<template>
<div class="fa_piao_cha_yan">
  <!-- :action="doUpload"    :data="pppss"-->
  <el-row>
    <h2 style="color:#048782;font-size:30px">发票查验机器人</h2>
    <!-- <p><h5 style="align: right;"><a href="@\public\云平台新增单位用户机器人绑定.mp4">视频</a></h5> -->
      ...
      <el-form label-width="120px" :model="editForm" ref="editForm">
        <el-row>
          ...
          <el-col :span="12">
              <!-- <el-form-item label="日期"> -->
              <!-- <el-input size="small" v-model="editForm.created_time" ></el-input> -->
              <el-button type="primary" round v-if="editForm.file_state === '1'" @click="start_robot(editForm)">启动机器人</el-button>
              <el-button type="info" round v-if="editForm.file_state === '2'" disabled>运行中</el-button>
              <el-button icon="el-icon-download" type="danger" round v-if="editForm.file_state === '3'" @click="rusult_robot(editForm)">查看结果</el-button>
            <!-- </el-form-item> -->
          </el-col>
        </el-row>
      </el-form>
...
export default {
  ...
  data () {
    return {
      ...
      doUpload: '../robotapi/fa_piao_cha_yan/' + localStorage.yun_user.replace('"', '').replace('"', ''),
      editForm: {
      }
    }
  },
  created () {
    this.axios.get('../robotapi/fa_piao_cha_yan/' + this.pppss.name, {
    }).then((res) => {
      // if(res.data)
  ...
  methods: {
    ...
    rusult_robot (editForm) {
      const postUrl = '../robotapi/fa_piao_cha_yan_start/' + editForm.id
    ...
    start_robot (editForm) {
      ...
      const data = editForm
      console.log(id)
      this.axios.post('../robotapi/fa_piao_cha_yan_start/' + id, {
        data
      }).then((res) => {
        location.reload()
      })
